...
sub _find_binary_download_urls {
    ...
    my ($darwin_64)   = grep { $_->{name} =~ /darwin/      } @$assets;
    my ($freebsd_x64) = grep { $_->{name} =~ /freebsd/     } @$assets;
    ...
    $download_urls->{'darwin-x64'}  = $darwin_64->{browser_download_url}   if defined $darwin_64;
    $download_urls->{'freebsd-x64'} = $freebsd_x64->{browser_download_url} if defined $freebsd_x64;
